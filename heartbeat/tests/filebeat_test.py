...
def test_defaults():
    ...
    assert 'heartbeat test output' in c['readinessProbe']['exec']['command'][-1]
...
def test_adding_in_heartbeat_config():
    config = '''
heartbeatConfig:
  heartbeat.yml: |
    key:
...
'''
    ...
    assert 'heartbeat.yml' in c
    assert 'other-config.yml' in c

    assert 'nestedkey: value' in c['heartbeat.yml']
    assert 'dot.notation: test' in c['heartbeat.yml']
    ...
    assert {'mountPath': '/usr/share/heartbeat/heartbeat.yml', 'name': project + '-config', 'subPath': 'heartbeat.yml', 'readOnly': True} in d['containers'][0]['volumeMounts']
    assert {'mountPath': '/usr/share/heartbeat/other-config.yml', 'name': project + '-config', 'subPath': 'other-config.yml', 'readOnly': True} in d['containers'][0]['volumeMounts']
...
'''
    r = helm_template(config)
    assert r['daemonset'][name]['metadata']['labels']['app.kubernetes.io/name'] == 'heartbeat'
...
def test_setting_fullnameOverride():
    config = '''
fullnameOverride: 'heartbeat-custom'
'''
    r = helm_template(config)

    custom_name = 'heartbeat-custom'
